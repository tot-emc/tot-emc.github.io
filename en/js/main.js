...
function changeLanguage(lang) {
  ...
  // هدایت به مسیر صحیح
  if (lang === 'fa') {
    window.location.href = '/';  // به ریشه (فارسی)
  } else {
    window.location.href = '/en/';  // به انگلیسی
  }
}

// تنظیم لینک فعال پس از بارگذاری صفحه
document.addEventListener('DOMContentLoaded', function () {
  const isEnglish = window.location.pathname.startsWith('/en/');
  const currentLang = isEnglish ? 'en' : 'fa';

  document.querySelectorAll('.lang-link').forEach(link => {
    if ((link.textContent === 'EN' && isEnglish) ||
        (link.textContent === 'FA' && !isEnglish)) {
      link.classList.add('active');  // یا link.style.opacity = 0.5;
    }
  });
});
